...
class ChunkingPlayground:
    ...
    def setup_left_panel(self, parent):
        ...
        # Sección 3: Configuración Agentic
        self.agentic_frame = ttk.LabelFrame(parent, text="🤖 Configuración Agéntica", padding=5)
        self.agentic_frame.pack(fill=tk.X, pady=(0, 10))

        # Proveedor LLM
        ttk.Label(self.agentic_frame, text="Proveedor LLM:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.agentic_provider_var = tk.StringVar(value="local")  # Default a local
        provider_frame = ttk.Frame(self.agentic_frame)
        provider_frame.grid(row=0, column=1, sticky=tk.EW, padx=(10, 0), pady=2)

        providers = [("LLM Local", "local"), ("Google Gemini", "gemini")]
        for i, (text, value) in enumerate(providers):
            ttk.Radiobutton(provider_frame, text=text, variable=self.agentic_provider_var,
                            value=value).grid(row=0, column=i, sticky=tk.W, padx=(0, 10))
        ...
        self.agentic_process_button.grid(row=1, column=0, columnspan=2, pady=(10, 0), sticky='ew')
        
        self.agentic_frame.columnconfigure(1, weight=1)
        
        # Inicialmente ocultar la configuración agentic
        self.agentic_frame.pack_forget()
    ...
    def toggle_agentic_config(self):
        ...
        try:
            if self.strategy_var.get() == 'agentic':
                self.agentic_frame.pack(fill=tk.X, pady=(0, 10))
                self.set_status("Modo Agentic seleccionado. Ajusta los parámetros y pulsa 'Procesar con LLM'.")
            else:
                self.agentic_frame.pack_forget()
        except Exception as e:
            logger.error(f"Error in toggle_agentic_config: {traceback.format_exc()}")
