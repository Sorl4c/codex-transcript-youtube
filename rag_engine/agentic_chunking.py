...
def _parse_llm_response(response_text: str) -> Dict[str, Any]:
    ...
    # 2. Buscar el primer bloque {...} grande (aunque no esté en markdown)
    json_block = re.search(r"(\{[\s\S]*\})", response_text)
    if json_block:
        try:
            return json.loads(json_block.group(1))
        except json.JSONDecodeError:
            pass
    # 3. Limpiar posibles caracteres extraños y reintentar
    ...
    try:
        ...
        pass
    # 4. Si todo falla, mostrar los primeros 500 caracteres para debug
    raise Exception(f"No se pudo parsear la respuesta JSON: {response_text[:500]}...")
